...
async function startServer() {
  ...
  app.use(helmet({
    // Allow inline styles for the SPA; tighten further if a nonce strategy is adopted.
    contentSecurityPolicy: false,
    // Allow embedding in same-origin iframes (for the console's own preview panels).
    frameguard: { action: "sameorigin" },
  }));

  // CORS: restrict API access to the app's own origin in production.
  const appUrl = process.env.APP_URL || "http://localhost:3000";
  app.use(cors({
    origin: process.env.NODE_ENV === "production"
      ? [appUrl, ...(process.env.CORS_ALLOWED_ORIGINS?.split(",").map(s => s.trim()) || [])]
  ...
  const apiLimiter = rateLimit({
    ...
    skip: (req) => {
      ...
      return req.path.startsWith("/assets/") || req.path === "/api/trpc/health.ping";
    },
  });

  // Strict upload limiter: 5 uploads per 10 minutes per IP.
