...
const PROMPT_TABS: PromptTab[] = [
  {
    ...
    label: "Pass 1: Layout Analysis",
    category: "pipeline",
    icon: Zap,
    description: "Instructions for the local VLM to identify bounding boxes and element types on each page.",
    variables: ["{{image_url}}", "{{page_number}}", "{{game_system}}"],
  },
  {
    name: "pass2_content_extraction",
    label: "Pass 2: Content Extraction",
    category: "pipeline",
    icon: Database,
    description: "Instructions for cloud LLMs to extract structured JSON from TTRPG page content.",
    variables: ["{{image_url}}", "{{json_schema}}", "{{layout_metadata}}", "{{lexicon_terms}}"],
  },
  {
    ...
    category: "pipeline",
    icon: RefreshCw,
    description: "Instructions for the adversarial referee that resolves multi-model discrepancies.",
    variables: ["{{source_image_url}}", "{{model_results}}", "{{json_schema}}", "{{confidence_threshold}}"],
  },
  {
